...
export class UniswapV3 extends BaseDexService {
  ...
  getTokenData(contractAddress: string, blockNumber: number | undefined | null = null): Promise<TokenData | null> {
  ...
  getPoolsData(pools: string[] = []): Promise<PoolData[] | []> {
    return fetchPoolsData(this.client, pools.map(poolAddress => poolAddress.toLowerCase()));
  }

  getPoolsPastData(pools: string[], blockNumber: number): Promise<PoolData[]> {
    return fetchPoolsPastData(this.client, pools.map((poolAddress: string) => poolAddress.toLowerCase()), blockNumber);
  }

  getPoolsDayDatas(pools: string[], startTime: number, skip: number = 0): Promise<PoolDayData[]> {
    return fetchPoolsDayData(this.client, pools.map((poolAddress: string) => poolAddress.toLowerCase()), startTime, skip);
  }

  getEthPrice(blockNumber: number | undefined | null = null): Promise<BigNumber> {
    return blockNumber
