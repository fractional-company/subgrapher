...
describe('UniswapV3 integration tests', () => {
  ...
  describe('Token Data', () => {
    ...
    it('should return historic data for token DOG', async () => {
      const mock = {
        ...
        untrackedVolumeUSD: new BigNumber('52742716.81743021625159454334182219'),
    ...
    it('should return 1 token pool - 0xe356025459d8222b068c474d50d93933f7316610', async () => {
      const pools = await service.getTokenPools('0x671ee71e1d285606c784ae9f262bc0d87e315e6c');
      expect(pools.length).toEqual(1);
      const pool = pools[0];
      expect(pool.address).toEqual('0xc260d73773a1e0cd7c294310f871fdc1cfbf9095');
  ...
  describe('Pool Data', () => {
    it('should return pool data for Half Suit Meebit', async () => {
      const poolsData = await service.getPoolsData(['0xc260d73773a1e0cd7c294310f871fdc1cfbf9095']);
    ...
    it('should return past pool data for Half Suit Meebit', async () => {
      const poolsData = await service.getPoolsPastData(['0xc260d73773a1e0cd7c294310f871fdc1cfbf9095'], 13401000);
      const poolData = poolsData[0];
      expect(poolData.address).toEqual('0xc260d73773a1e0cd7c294310f871fdc1cfbf9095');
      expect(poolData.token0.address).toEqual('0x671ee71e1d285606c784ae9f262bc0d87e315e6c');
      ...
      expect(poolData.volumeUSD.toString()).toEqual('5090.869503999055973916256440568868');
      expect(poolData.token0Price.toString()).toEqual('2087.112107191257175815740566857251');
      expect(poolData.token1Price.toString()).toEqual('0.0004791309468017775090548934267174422');
      expect(poolData.totalValueLockedToken0.toString()).toEqual('10233.206769724493713198');
      expect(poolData.totalValueLockedToken1.toString()).toEqual('0.004106219564178342');
    });

    it('should return past day data for LP of Half Suit Meebit vault', async () => {
      const poolsData = await service.getPoolsDayDatas(['0xc260d73773a1e0cd7c294310f871fdc1cfbf9095'], 13401000);

      let poolData1 = poolsData.find((i: PoolDayData) => i.date === 1629244800);
      let poolData2 = poolsData.find((i: PoolDayData) => i.date === 1629417600);
      let poolData3 = poolsData.find((i: PoolDayData) => i.date === 1629590400);
    ...
    it('should return bulk token pools data with weights (DOG, Half Suit Meebit)', async () => {
      const poolsDatas = await service.getBulkPoolsData({
        [DOG_ADDRESS]: ['0xd0a927837cff32d55e5d8fad5145562e7ad85f30', '0x7731ca4d00800b6a681d031f565deb355c5b77da'],
  ...
  describe('ETH price', () => {
    ...
    it('should return past ETH price', async () => {
      ...
      expect(ethPrice.toString()).toEqual('3285.337343994361572420319851159309');
    });
